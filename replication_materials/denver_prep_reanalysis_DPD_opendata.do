...
*********************************************************************************************************
* Import, clean and merge the Denver Open Police Department reported crimes dataset

import delimited "$denver/crime2016.csv", clear
save  "$denver/crime2016.dta", replace 
import delimited "$denver/crime2017.csv", clear
save  "$denver/crime2017.dta", replace 
import delimited "$denver/crime2018.csv", clear
save  "$denver/crime2018.dta", replace 
import delimited "$denver/crime2019.csv", clear
save  "$denver/crime2019.dta", replace 
import delimited "$denver/crime2020.csv", clear
save  "$denver/crime2020.dta", replace 

append using    "$denver/crime2019.dta"
append using    "$denver/crime2018.dta"
append using    "$denver/crime2017.dta"
append using    "$denver/crime2016.dta"

save "$denver/open_data_crime.dta", replace
use  "$denver/open_data_crime.dta", clear
	set more off
...
* Clean up the date variable
	...
	tabstat crime_VC_nosexual_alt crime_NVC crime_VC_norape crime_sexual_aslt crime_agg_aslt_alt crime_assault , s(sum) by(year)
...
* combine Stapleton (61) with Central Park (11), and recode neighborhoods >62 so that range = 1-78
	...
	/* No longer needed in this version: recode neighborhood (61 = 11)
...
* I.E., "None" and "Stapleton" do not appear here.
	...
	merge 1:1 nh_week using "$denver/merged_data_for_reanalysis_new_data.dta", gen(_merge3)
...
* Replace missings with zeros (N = 1483)
	...
	// Intermediate Save
	save "$denver/merged_data_for_reanalysis_new_data_pre_traffic.dta", replace
...
* Merge & save
	merge 1:1 nh_week using "$denver/merged_data_for_reanalysis_new_data.dta", gen(_merge4)
...
*** Save was missing -- added a line here:
 	save "$denver/merged_data_for_reanalysis_new_data.dta", replace
	save "$denver/merged_data_for_reanalysis_new_data_pre_aqi.dta", replace
...
// DATASETS
use "$denver/merged_data_for_reanalysis_new_data.dta", clear
...
// Intermediate save
	...
	save "$denver/merged_data_for_reanalysis_new_data_pre_acs.dta", replace
	use  "$denver/merged_data_for_reanalysis_new_data_pre_acs.dta", replace
...
* Merge data 
	...
	save "$denver/merged_data_for_reanalysis_new_data.dta", replace
	save "$denver/post_acs_merged_data_for_reanalysis_new_data.dta", replace
	use  "$denver/post_acs_merged_data_for_reanalysis_new_data.dta", clear
	set more off
...
* Save and move on to the analysis R script or figure .do file
	...
	save "$denver/combined_reanalysis_new_data.dta", replace
	use  "$denver/combined_reanalysis_new_data.dta", clear
